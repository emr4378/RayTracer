...
void Camera::setDimensions(float wid, float hei, float zFar, float pSize) {
	setWidth(wid);
	setHeight(hei);
	setDistance(zFar);
	setPixelSize(pSize);
...
void Camera::setPerspective(float fov, float aspect, float zNear, float pSize) {
...
Colour * Camera::render(float sWid, float sHei) {
	if (world == 0) {
		return 0;
	}
	...
	World& w = *world;
	int rows = ceil(sHei/pixelSize);
	int cols = ceil(sWid/pixelSize);
	...
	float xInc = width / cols;
	float yInc = height / rows;
...
Colour Camera::illuminate(Object3D * prevHit, const Ray& oRay, int depth) {
	...
	for (int s = 0; s < samples; s++) {
		...
		if (closest != 0) {
			...
			if (depth > 0) {
				float refl = oIntData.material.getReflectiveCoeff();
				float transm = oIntData.material.getTransmissiveCoeff();
				...
				if (transm > 0) {
					float n = oIntData.material.getRefractionIndex();
...
void Camera::setPixelSize(float p) {
	pixelSize = p;
}
