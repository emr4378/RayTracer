...
float pixSize = 1;			//the size of pixels in the resulting image. Awesome for testing purposes.
bool superSample = false;	//enable/disable super-sampling
float fov = 60.0f;			//the field of view of the camera
...
void drawPixel(int x, int y, float xSize, float ySize, Colour * col) {
 ...
 */
void checkpoint(int n, bool useWard, float lMax, float ldMax) {
	Sphere * _small;
	Sphere * _big;
	Polygon3D * _floor;
	...
	_small = new Sphere( -4, -5.0f/3.125f, -18, 3.125, sMat);
	...
	if (n >= 3) {
		...
		for (iter = world.objectList.begin(); iter != world.objectList.end(); iter++) {
			(*iter)->getMaterial()->setAmbientCoeff(0.3f);
		}
